...
def compute_decoupled_policy_loss(ewma_log_prob, old_log_prob, log_prob, advantages, eos_mask, cliprange):
    """Adapted from https://github.com/huggingface/trl/blob/main/trl/trainer/ppo_trainer.py#L1122

    Args:
        ewma_log_prob: `(torch.Tensor)`
            shape: (bs, response_length)
        old_log_prob: `(torch.Tensor)`
            shape: (bs, response_length)
        log_prob: `(torch.Tensor)`
    ...
    """
    coeff = torch.exp(ewma_log_prob - old_log_prob)
    negative_approx_kl = log_prob - ewma_log_prob
    ratio = torch.exp(negative_approx_kl)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, eos_mask)
    ...
    pg_loss = verl_F.masked_mean(coeff * torch.max(pg_losses, pg_losses2), eos_mask)
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses).float(), eos_mask)
    return pg_loss, pg_clipfrac, ppo_kl